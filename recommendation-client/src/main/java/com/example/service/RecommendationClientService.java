...
public class RecommendationClientService {
    ...
    //通过restTemplate调用user-service的接口，获取用户的详细信息
    ...
    public CompletableFuture<RecommendationDTO> getRecommendationData(Long userId,String gender,
            String age,String occupation,String zipcode){
        ...
        ResponseEntity<Collection<RecommendedMovieDTO>> recommendedMovies =
                restTemplate.exchange("http://recommendation-service/recommendation/recommend/user/" +
                        userId.toString(), HttpMethod.GET, null, ptrRecommendedMovie);
        ...
        CompletableFuture<RecommendationDTO> test = getUserDetails(userId).thenCombine(getMovies(ids), (user, movies)->{
            ...
            movies.stream().forEach((recommendation) -> {
                String name =  recommendation.getTitle();
                String genres =  recommendation.getGenres();
...
//                        .get()
//                        .getLikes();
                data.getMovies().add(new MovieDTO(movieId.toString(), name, genres));
            });
            return data;
        });
        return test;
